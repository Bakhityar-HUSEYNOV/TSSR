...
if [ $? -ne 0 ] ; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        echo -e "${RED}Failed to install curl, check the logs for errors" 
        echo -e "${RED}DEBUG: Failed to install curl, check the logs for errors" >>logs/tailscale_install.log
        exit 1
    fi
fi
...
if [ $? -eq 0 ] ; then
    ...
    echo -e "${GREEN}DEBUG: Tailscale is already installed " >>logs/tailscale_install.log
else
    ...
    if [ $? -eq 0 ] ; then
        ...
        echo -e "${RED}Failed to install Tailscale, check the logs for details"
        echo -e "${RED}Failed to install Tailscale, check the logs for details" >>logs/tailscale_install.log
        exit 1
    fi
fi
...
if tailscale status | grep -q "Logged out" ; then
    ...
    echo -e "${GREEN}DEBUG:Tailscale is already authenticated, no need to log in again" >>logs/tailscale_install.log
    exit 0
fi
...
if [ -f /tmp/key.txt ] ; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        echo -e "${GREEN}Tailscale is now running, your ip address is: $(tailscale ip -4)" >>logs/tailscale_install.log
    else
        echo -e "${RED}DEBUG: Failed to authenticate Tailscale, check the logs for details" >>logs/tailscale_install.log
        echo -e "${RED}Failed to authenticate Tailscale, check the logs for details" 
        exit 1
    fi
else
    echo -e "${RED}No secret auth key found, please provide one in top_secret.txt"
    echo -e "${RED}DEBUG: No secret auth key found, please provide one in top_secret.txt" >>logs/tailscale_install.log
    exit 1
fi